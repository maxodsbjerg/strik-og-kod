...
strik_bigrams_filtered %>% 
  filter(lplace == "København") %>% 
  filter(str_detect(word2, "\\bstrik[a-zæø]*")) %>% 
  count(word1, word2, sort = TRUE) -> strikkeord_bigrams_counts_kbh
```
...
bigram_graph_kbh <- strikkeord_bigrams_counts_kbh %>%
  filter(n > 6) %>%
  graph_from_data_frame()
```
...
ggraph(bigram_graph_kbh, layout = "fr") +
  geom_edge_link(aes(edge_alpha = n), show.legend = FALSE,
                 arrow = a, end_cap = circle(.07, 'inches')) +
  geom_node_point(color = "#f7a1bd", size = 5) +
  geom_node_text(aes(label = name), vjust = 1, hjust = 1) +
...
```{r}
ggsave("strikke_bigrams.png", width = 28, height = 20, units = "cm", bg = "white")
```
